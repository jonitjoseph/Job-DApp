...
contract CIEG {
  ...
  function addPerfMatrix(address _vEmpAddress, address payable _candAddress, uint256 _enrolledJobId, uint256 _evalScore) public payable {
      require(msg.value>0);
      require(msg.sender == _vEmpAddress, "Employer only can add Performance Matrix!");
      uint256 enrldJobId = _enrolledJobId;
      perfMatrix[enrldJobId] = PerformanceMatrix(_vEmpAddress, _candAddress, _enrolledJobId, _evalScore, msg.value);
      _candAddress.transfer(msg.value);
  }
  ...
  function getPerfMatrix(uint256 _jobId) external view returns(address, address ,uint256, uint256, uint256) {
    return (
        perfMatrix[_jobId].candAddress,
        perfMatrix[_jobId].vEmpAddress,
        perfMatrix[_jobId].enrolledJobId,
        perfMatrix[_jobId].evalScore,
        perfMatrix[_jobId].paidAmount
    );
  }
